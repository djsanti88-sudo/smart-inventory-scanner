...
| # | Risk | Sev | Likelihood | Mitigation | Approval | Status |
|---|------|-----|-----------|------------|----------|--------|
| 1 | 163 local-only commits (whole ladder + size-merge era) could be lost to disk/OS failure; nothing is pushed | High | Low | Push/PR when the owner gives the word (push is owner-gated by standing rule); local git history intact; loved-baseline preview deployment preserves the build output | Owner word needed to push | **[RESOLVED 2026-07-29 — branch train merged via PRs #12-#21; see PROGRESS.md]** |
| 2 | Production still runs the pre-ladder build (the `vercel.json` auto-deploy flag is removed as of PR #21, but the Vercel dashboard Git connection for Production Branch = `master` is still a pending owner action, so merges do not yet deploy); customers do not have the proven fixes | Med | Certain until promoted | Go-live checklist: owner sign-off + production env keys + `vercel promote`, or completing the dashboard Git connection; preview `inventory-5tk3c3vxf` is the validated candidate | Owner sign-off | Open |
...
## Corpus licensing + branch hazards (2026-07-29)

| # | Risk | Sev | Likelihood | Mitigation | Approval | Status |
|---|------|-----|-----------|------------|----------|--------|
| 1 | The ~4M-row retail corpus incorporates Open Food Facts data, which is licensed ODbL (Open Database License) - a share-alike/attribution copyleft license. This obligation is currently UNTRACKED: no attribution is shown in-app, and no legal review has confirmed whether ODbL's share-alike terms are compatible with charging customers for a product built on this data | High | Certain (obligation exists regardless of enforcement) | Needs professional legal review of ODbL compliance (attribution surface, share-alike scope, commercial-use terms) BEFORE any paid go-live that exposes this corpus to customers | Owner + legal review needed | Open |
| 2 | Branch `benchmark-tire-db-automation` is PARKED by owner order and must never be deleted or merged - merging it would delete ~152k lines including the poison guard that protects corpus integrity | High | Low (standing order documented, but any automated branch-cleanup pass is a risk vector) | Standing owner order recorded here and in BACKLOG.md ("DO NOT touch `benchmark-tire-db-automation`"); any branch-pruning task must explicitly exclude it | Owner order (no further approval needed to respect it) | Accepted / standing hazard |
