...
describe("D1: every unknown scan enqueues its ledger writes and is never fake-synced", () => {
  ...
  it("the counted unknown feed row is NOT stamped synced and carries the same-fact quantityDelta 1", () => {
    const store = offlineStore();
    store.getState().processScan("888888888882");
...
Expected: FAIL. Test 1 fails because `pendingSyncQueue` contains no `SAVE_PRODUCT`/`INCREMENT_COUNT` for the provisional (nothing is enqueued in `ensureProvisionalCount`); test 2 fails because the row is stamped `syncStatus: "synced"` and carries `quantityDelta: 0`; test 5 fails because `provenanceTier` does not exist yet.
