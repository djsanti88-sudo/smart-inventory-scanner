...
- [ ] **Step 3:** For matched rows, fill blank `brand`/`model`/`size`/`manufacturer_part_number` in `tires` from boss values (blank-only fill; NEVER overwrite non-blank), insert provenance rows, log every write to `remaining_blank_fill_audit` with `trust_color='green'`, `action='boss_truth_fill'`. CREATE the `provenance` table here if missing (this task owns the schema; A5 backfills): `provenance(id INTEGER PRIMARY KEY, product_id TEXT, barcode TEXT, source_name TEXT, source_ref TEXT, sheet TEXT, row TEXT, batch_id TEXT, imported_at TEXT, evidence_level TEXT, license_note TEXT, content_hash TEXT, UNIQUE(product_id, barcode, source_name, source_ref, sheet, row))` - all inserts are UPSERTs so re-runs never duplicate. Multi-barcode write rule: barcode-matched rows fill ONLY the `tires` row with that exact barcode; part-number-matched products fill sibling rows only where the field is blank AND non-blank siblings do not disagree - any sibling disagreement routes the row to the review CSV instead.
...
- [ ] **Step 1:** Use the EXISTING `tire_product_part_number_aliases` schema exactly as-is (per adversarial schema inspection it has `normalized_part_number`, `display_part_number`, `source`, `trust_color`, `confidence_score`, `is_unambiguous` and non-null constraints - inspect and honor them; do NOT invent columns like alias_value/alias_kind). Map: display = raw variant, normalized = normPartKey(variant), source = evidence origin, trust_color = green for corroborated, is_unambiguous = 1 only for single-product mappings. Document any unavoidable ALTER.
...
- [ ] **Step 1:** Use the `provenance` table created by A3 (create with A3's exact schema if running standalone). Backfill known origins: boss rows (from A3), rows present in live-Turso snapshot lineage if identifiable, corpus-import batches identifiable from `process_merge_audit`. Evidence levels: `boss_source` > `import_audit` > `unknown`.
...
- [ ] **Step 1:** Score per lane: coverage (fields filled / fields asked), accuracy on answer-key rows (exact-match after normalization; size compared structurally e.g. 265/70R17; MPN truth is weak-confidence - label its accuracy advisory), WRONG-ANSWER RATE (filled but != truth - the killer metric), mean latency, ACTUAL usage from each lane report (Firecrawl operations, Codex invocation), cost per successful fill, trusted-host share, automation-friendliness (scriptability, rate limits, structured output reliability - scored 1-5 with one-line justification each).
...
- [ ] **Step 1:** Generate idempotent staging SQL (CREATE TABLE staging_*, INSERT batches) to files, not to Turso - covering ALL five datasets: tires, tire_part_numbers (active), tire_product_part_number_aliases, canonical_tire_products, provenance. Include the relationship/runtime-lookup gate queries to run AGAINST STAGING, an atomic promotion script (rename-swap in one transaction batch), and a pre-promotion backup step (dump current live tables) with rollback commands.
