...
import { decodeWithGpt, gptTierFor, GPT_DECODE_WORST_CASE_USD } from "./gptDecodeClient";
...
function captureFetch() {
  let sentBody: CapturedRequestBody = {};
  const fetchImpl = (async (_url: Parameters<typeof fetch>[0], init?: RequestInit) => {
    sentBody = JSON.parse(String(init?.body)) as CapturedRequestBody;
...
describe("decodeWithGpt", () => {
  test("default abort cap is 35 seconds (owner-set 2026-07-06; exhaustion band tops out ~28s)", async () => {
    ...
    vi.useFakeTimers();
    try {
      const p = decodeWithGpt("848983006257", { apiKey: "k", fetchImpl: f });
      elapsed = 34_900;
      ...
      expect(r.usdComputedFloor).toBe(0.39);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  test("sends the exact 21/21 config and parses a strong answer to verified", async () => {
    ...
    const r = await decodeWithGpt("848983006257", { apiKey: "k", fetchImpl: f });
    expect(r.tier).toBe("verified");
    expect(r.productName).toContain("Wildpeak");
    expect(r.searches).toBe(2);
    expect(r.usdComputedFloor).toBeCloseTo((3000 / 1e6) * 0.75 + (900 / 1e6) * 4.5 + 0.02, 5);
  ...
  test("prompt v3 forbids invented products and drops the always-answer clause", async () => {
    const f = okFetch(respBody(MODEL_JSON));
    await decodeWithGpt("848983006257", { apiKey: "k", fetchImpl: f });
    const body = JSON.parse(vi.mocked(f).mock.calls[0][1]!.body as string);
  ...
  test("honest-empty: an empty productName is tier none (not a suggestion), category still surfaced", async () => {
    ...
    const r = await decodeWithGpt("049000000000", { apiKey: "k", fetchImpl: f });
  ...
  describe("wave-3 Item E: 4xx pre-execution rejections bill $0, not worst case (2026-07-20 owner-approved)", () => {
    test.each([400, 401, 403, 404, 422])("HTTP %i is rejected pre-execution: usdComputedFloor is 0 (nothing was billable)", async (status) => {
      const f = vi.fn(async () => ({ ok: false, status, json: async () => ({ error: { message: "rejected" } }) })) as unknown as typeof fetch;
      const r = await decodeWithGpt("049000000000", { apiKey: "k", fetchImpl: f });
      expect(r.tier).toBe("none");
      expect(r.usdComputedFloor).toBe(0);
    ...
    test("401 sets the exact honest error message for the ladder's skip reason", async () => {
      ...
      const r = await decodeWithGpt("049000000000", { apiKey: "bad-key", fetchImpl: f });
    ...
    test.each([429, 500, 502, 503])("HTTP %i still bills worst case (may have executed or is ambiguous per cost-truth rule)", async (status) => {
      ...
      const r = await decodeWithGpt("049000000000", { apiKey: "k", fetchImpl: f });
      expect(r.tier).toBe("none");
      expect(r.usdComputedFloor).toBe(GPT_DECODE_WORST_CASE_USD);
    });
    ...
    test("abort still bills worst case, unaffected by the 4xx carve-out", async () => {
      ...
      const r = await decodeWithGpt("049000000000", { apiKey: "k", fetchImpl: f, timeoutMs: 20 });
      expect(r.aborted).toBe(true);
      expect(r.usdComputedFloor).toBe(GPT_DECODE_WORST_CASE_USD);
    });
  });

  test("abort: fetch rejecting with AbortError -> aborted true, usdComputedFloor = worst case", async () => {
    const f = vi.fn(async (_u: string, init: RequestInit) => {
      return await new Promise((_res, rej) => {
        init.signal?.addEventListener("abort", () => rej(Object.assign(new Error("aborted"), { name: "AbortError" })));
      });
    }) as unknown as typeof fetch;
    const p = decodeWithGpt("049000000000", { apiKey: "k", fetchImpl: f, timeoutMs: 20 });
    const r = await p;
    expect(r.aborted).toBe(true);
    expect(r.usdComputedFloor).toBe(0.39); // billed server-side anyway: count worst case
    expect(r.tier).toBe("none");
  });
  ...
  describe("wave-3: external signal threading (2026-07-20 owner-ratified)", () => {
    test("aborts the HTTP call when an externally-passed signal fires, even before the internal 35s timeout", async () => {
      ...
      const p = decodeWithGpt("848983006257", { apiKey: "k", fetchImpl: f, signal: external.signal });
      external.abort();
      const r = await p;
      expect(r.aborted).toBe(true);
    ...
    test("with no external signal passed, behaves exactly as before (internal 35s timeout still governs)", async () => {
      ...
      const r = await decodeWithGpt("848983006257", { apiKey: "k", fetchImpl: f, timeoutMs: 10 });
      expect(r.aborted).toBe(true);
    });
  });
  ...
  test("Z4: prompt teaches GTIN zero-padding equivalence (code property, not an answer hint)", async () => {
    ...
    await decodeWithGpt("0049000006346", { apiKey: "k", fetchImpl: captured.fetchImpl });
  ...
  describe("G2: GPT_DECODE_MODEL env override", () => {
    afterEach(() => { delete process.env.GPT_DECODE_MODEL; });
    test("defaults to gpt-5.4-mini when unset", async () => {
      const c = captureFetch();
      await decodeWithGpt("049000006346", { apiKey: "k", fetchImpl: c.fetchImpl });
      expect(c.body().model).toBe("gpt-5.4-mini");
    });
    test("uses the env model when set", async () => {
      process.env.GPT_DECODE_MODEL = "gpt-6-preview";
      const c = captureFetch();
      await decodeWithGpt("049000006346", { apiKey: "k", fetchImpl: c.fetchImpl });
  ...
  test("G1: requests structured output via json_schema so non-JSON replies are impossible", async () => {
    const captured = captureFetch();
    await decodeWithGpt("049000006346", { apiKey: "k", fetchImpl: captured.fetchImpl });
    const format = captured.body().text?.format;
    expect(format?.type).toBe("json_schema");
    expect(format?.name).toBe("product_identity");
