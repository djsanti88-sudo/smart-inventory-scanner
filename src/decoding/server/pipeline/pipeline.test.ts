...
vi.mock("@/decoding/server/cache/decodeCacheStore", () => ({
  getPersistedDecode: vi.fn(),
  persistDecode: vi.fn(),
}));
vi.mock("@/decoding/server/pipeline/storage", () => ({ decodeStorage: vi.fn() }));
...
import { getPersistedDecode, persistDecode } from "@/decoding/server/cache/decodeCacheStore";
...
} from "@/decoding/server/pipeline/pipeline";
