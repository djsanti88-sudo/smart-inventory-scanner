...
import { getKnowledgeDb } from "@/decoding/server/knowledge/knowledgeDb";
...
export interface RetailLookupOutcome {
  result: RetailLookupResult | null;
...
// Driver construction + credential reading live in @/decoding/server/tursoClient; the cache cell, the
...
async function lookupTurso(code: string): Promise<RetailLookupOutcome> {
  ...
  try {
    ...
    return { result: null, status: "turso_miss" };
  } catch (e) {
    console.warn("[retail-knowledge] Turso query error:", (e as Error).message);
    return { result: null, status: "turso_error" };
  }
}
...
export async function lookupRetailBarcodeWithStatusAsync(code: string): Promise<RetailLookupOutcome> {
  const sqliteResult = lookupSqlite(code);
  if (sqliteResult) return { result: rejectIfExampleOrTestRow(sqliteResult), status: "sqlite_hit" };
  const outcome = await lookupTurso(code);
  return { ...outcome, result: rejectIfExampleOrTestRow(outcome.result) };
}

export async function lookupRetailBarcodeAsync(code: string): Promise<RetailLookupResult | null> {
  return (await lookupRetailBarcodeWithStatusAsync(code)).result;
}
