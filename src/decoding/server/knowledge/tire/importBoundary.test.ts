...
// so NO client-bundled module may import src/decoding/server/knowledge/tire. This walks src/ and fails if any
// "use client" file, client component, or Zustand store imports @/decoding/server/knowledge/tire or the generated
// index. (The `server-only` package enforces this at build time too; this test fails fast + explains why.)

const SRC = join(process.cwd(), "src");
const FORBIDDEN = /@\/decoding\/server\/knowledge\/tire|tireKnowledge\.generated|TireKnowledgeProvider|tireKnowledgeIndex/;
...
describe("server-only boundary: tire-knowledge is never client-bundled", () => {
  ...
  it("the index reader + provider declare the server-only boundary", () => {
    for (const f of ["tireKnowledgeIndex.ts", "TireKnowledgeProvider.ts"]) {
      const src = readFileSync(join(SRC, "decoding", "server", "knowledge", "tire", f), "utf8");
