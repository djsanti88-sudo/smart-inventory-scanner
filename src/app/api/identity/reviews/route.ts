...
type ReviewRepository = Pick<LocalIdentityRepository, "listIdentityReviews"> & Partial<Pick<LocalIdentityRepository, "pageIdentityReviews" | "listCurrentIdentityLinks" | "pageCurrentIdentityLinks" | "applyReviewAction" | "resolveIdentityReview" | "saveIdentityLink" | "createTenantProduct" | "revokeIdentityLink">>;
...
async function defaultPageCurrentApprovedLinks(businessId: string, input: { page: number; pageSize: number }) { const all = await defaultCurrentApprovedLinks(businessId), start = (input.page - 1) * input.pageSize; return { items: all.slice(start, start + input.pageSize), total: all.length }; }
export const GET = createIdentityReviewRoute({ enabled: isLocalIdentityApplyEnabled, authorize: async (_request, businessId) => actorFromEnvironment(businessId), repository: defaultRepository, currentVersions: defaultVersions, currentModel: defaultModel, currentApprovedLinks: defaultCurrentApprovedLinks, pageCurrentApprovedLinks: defaultPageCurrentApprovedLinks });
