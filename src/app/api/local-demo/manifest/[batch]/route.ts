...
export { computeLocalDemoBatchHashes } from "@/server/localDemoArtifacts";
...
const BATCH_KEYS = [
  "schemaVersion", "seed", "gitSha", "databaseSha256", "batch", "agent",
...
function isRow(value: unknown, batch: number): value is LocalDemoBatchRow {
  ...
  return trusted &&
    ...
    [
      "barcodeType", "brand", "model", "size", "loadIndex", "speedRating",
      "manufacturerPartNumber", "type", "season", "confidence",
      "currentStatus", "usableFor", "angle", "stratum",
    ].every(
      (key) => typeof value[key] === "string" &&
        (value[key] as string).length <= 500,
    );
}
  ...
  return async function GET(
    ...
    if (!/^(?:0[1-9]|[12][0-9]|30)$/.test(batchText)) {
      return NextResponse.json(
        { error: "Invalid batch", localDemo: true },
        { status: 400, headers: localDemoNoStoreHeaders },
      );
    }
    try {
      const batch = Number(batchText);
      const active = readContainedJson(
        options.reportsRoot,
        resolve(options.reportsRoot, "active-run.json"),
        16 * 1024,
      );
      if (
        !isRecord(active) ||
        !hasExactKeys(active, ACTIVE_KEYS) ||
        active.schemaVersion !== 1 ||
        typeof active.runDirectory !== "string" ||
        active.runDirectory.length < 1 ||
        active.runDirectory.length > 200 ||
        typeof active.gitSha !== "string" ||
        !/^[a-f0-9]{7,64}$/i.test(active.gitSha) ||
        !isSha256(active.databaseSha256) ||
        !isSha256(active.manifestSha256) ||
        typeof active.generatedAt !== "string" ||
        !Number.isFinite(Date.parse(active.generatedAt))
      ) {
      ...
      if (
        ...
        manifest.gitSha !== active.gitSha ||
        manifest.databaseSha256 !== active.databaseSha256 ||
        manifest.manifestSha256 !== active.manifestSha256 ||
        typeof manifest.seed !== "string" ||
        manifest.seed.length < 1 ||
        manifest.seed.length > 200 ||
        manifest.generatedAt !== active.generatedAt ||
        manifest.total !== 3_000 ||
        manifest.batchSize !== 100 ||
        manifest.batchCount !== 30 ||
        manifest.agentCount !== 10 ||
        !Array.isArray(manifest.rows) ||
        manifest.rows.length !== 3_000 ||
        !manifest.rows.every((row, index) =>
          isRow(row, Math.floor(index / 100) + 1)
        ) ||
        !hasExactLocalDemoManifestAllocation(
          manifest.rows as LocalDemoBatchRow[],
        ) ||
        computeLocalDemoManifestHash(manifest) !== active.manifestSha256
      ) {
        throw new Error("Invalid active manifest");
      }
      const preflight = options.preflight();
      if (
        !isSha256(preflight.databaseSha256) ||
        preflight.databaseSha256 !== active.databaseSha256
      ) {
        throw new Error("Stale database");
      }
      const payload = readContainedJson(
        options.reportsRoot,
        resolve(runDirectory, "batches", `batch-${batchText}.json`),
        2 * 1024 * 1024,
      );
      if (
        !isRecord(payload) ||
        !hasExactKeys(payload, BATCH_KEYS) ||
        payload.schemaVersion !== 1 ||
        payload.gitSha !== active.gitSha ||
        payload.databaseSha256 !== active.databaseSha256 ||
        typeof payload.seed !== "string" ||
        payload.seed.length < 1 ||
        payload.seed.length > 200 ||
        payload.batch !== batch ||
        payload.agent !== Math.floor((batch - 1) / 3) + 1 ||
        payload.rowCount !== 100 ||
        !isSha256(payload.batchSha256) ||
        !isSha256(payload.expectedBarcodesSha256) ||
        !isSha256(payload.expectedCanonicalProductUidsSha256) ||
        !Array.isArray(payload.rows) ||
        payload.rows.length !== 100 ||
        !payload.rows.every((row) => isRow(row, batch))
      ) {
        throw new Error("Invalid batch payload");
      }
      const rows = payload.rows as LocalDemoBatchRow[];
      const manifestRows = manifest.rows.slice(
        (batch - 1) * 100,
        batch * 100,
      ) as LocalDemoBatchRow[];
      if (
        new Set(rows.map((row) => row.barcode)).size !== rows.length ||
        new Set(rows.map((row) => row.canonicalProductUid)).size !== rows.length ||
        JSON.stringify(rows) !== JSON.stringify(manifestRows)
      ) {
        throw new Error("Batch is not bound to the active manifest");
      }
      const hashes = computeLocalDemoBatchHashes(rows);
      if (
        hashes.batchSha256 !== payload.batchSha256 ||
        hashes.expectedBarcodesSha256 !== payload.expectedBarcodesSha256 ||
        hashes.expectedCanonicalProductUidsSha256 !==
          payload.expectedCanonicalProductUidsSha256
      ) {
