...
// POST) did `storage: await ladderStorage()` UNGUARDED. A storage init throw (e.g. Turso/libsql
...
// ladderStorage() throws on the FIRST call of each request (the rate-limit site under test) and returns
...
vi.mock("@/server/upc/storage", async (importOriginal) => {
  ...
  return {
    ...
    ladderStorage: vi.fn(async () => {
      ...
      return actual.fileLadderStorage(tmpLadderDir);
    }),
  };
});
...
describe("/api/ai-lookup rate-limit fail-open on storage init failure (Finding C)", () => {
  ...
  const keys = ["IS_E2E", "AI_LOOKUP_KILL_SWITCH", "AI_LOOKUP_DAILY_LIMIT", "GEMINI_API_KEY", "OPENAI_API_KEY", "FIRECRAWL_API_KEY", "BRAVE_SEARCH_API_KEY", "GO_UPC_API_KEY", "TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "DECODE_CACHE_FILE", "NEXT_PUBLIC_AUTH_MODE"];
  ...
  it("GET: a storage init throw at the rate-limit site does NOT 500 - the status endpoint still responds", async () => {
    failNextStorageInit = true; // the GET rate-limit ladderStorage() throws
