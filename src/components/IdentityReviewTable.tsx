...
function IdentityReviewQueue({ businessId, actorRole, previewDecisions = [] }: { businessId: string; actorRole: Role; previewDecisions?: readonly IdentityReviewRow[] }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/identity/reviews?businessId=${encodeURIComponent(businessId)}&bucket=${encodeURIComponent(bucket)}&page=${page + 1}&pageSize=${pageSize}`, { signal: controller.signal }).then(async (response) => {
      ...
      setReviews(loaded); setServerTotal(typeof (body as { total?: unknown })?.total === "number" ? (body as { total: number }).total : loaded.length); setStatus(loaded.length ? "Identity reviews loaded." : "No identity reviews need attention.");
  ...
  const pageCount = Math.max(1, Math.ceil((previewDecisions.length ? matching.length : serverTotal) / pageSize));
  const currentPage = Math.min(page, pageCount - 1);
  const visible = previewDecisions.length ? matching.slice(currentPage * pageSize, currentPage * pageSize + pageSize) : matching;
