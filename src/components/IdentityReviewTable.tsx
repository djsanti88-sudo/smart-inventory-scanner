...
function IdentityReviewQueue({ businessId, actorRole, previewDecisions = [] }: { businessId: string; actorRole: Role; previewDecisions?: readonly IdentityReviewRow[] }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/identity/reviews?businessId=${encodeURIComponent(businessId)}&bucket=${encodeURIComponent(bucket)}&page=${page + 1}&pageSize=${pageSize}`, { signal: controller.signal }).then(async (response) => {
      ...
      setReviews(loaded); setServerTotal(typeof (body as { total?: unknown })?.total === "number" ? (body as { total: number }).total : loaded.length); const totals = (body as { bucketTotals?: unknown })?.bucketTotals; if (totals && typeof totals === "object") setServerTotals(Object.fromEntries(buckets.map(({ key }) => [key, typeof (totals as Record<string, unknown>)[key] === "number" ? (totals as Record<string, number>)[key] : 0])) as Record<Bucket, number>); setStatus(loaded.length ? "Identity reviews loaded." : "No identity reviews need attention.");
