...
describe("GPT decode trust tiers - verified auto-count", () => {
  // D6 core (2026-07-20, decode-trust): the gptTrusted escape hatch is DELETED. mapGptDecodeResult
  ...
  it("gpt_self_report at confidence 0.9 on a PUBLIC BARCODE (upc_a) is demoted to a SUGGESTED row that still auto-applies + counts (identity shown, verified false, no approved alias)", async () => {
    ...
    const RESP = {
      ...
      // This mirrors what mapGptDecodeResult now ACTUALLY emits for a bare GPT self-report
...
describe("GPT decode trust tiers - suggested tier flows through unchanged", () => {
...
describe("GPT decode - weak guesses are normal candidates (info_only deleted, owner order 2026-07-06)", () => {
  ...
  it("skip-reason transparency: a skipped gpt-5.4-mini providerStatus entry is appended to decodeNote", async () => {
    const store = aiOnStore();
    ...
    const RESP = {
      ...
      providerStatuses: [
        { provider: "unexpected-provider", status: "ok", latencyMs: 10, sourceUrlsReturned: 0, exactCodeFound: false, identityFound: false },
        { provider: "gpt-5.4-mini", status: "skipped", latencyMs: 0, sourceUrlsReturned: 0, exactCodeFound: false, identityFound: false, errorCode: "budget_exceeded" },
      ],
    };
    const { restore } = stub(RESP);
    try {
...
describe("Direct idempotency lock (liveDecode re-entry on an already-resolved review)", () => {
  it("calling liveDecode twice for the SAME review is a no-op the second time (open-status guard, not the rescan/alias path)", async () => {
    ...
    // mirrors what mapGptDecodeResult actually emits post-demotion. It still auto-APPLIES onto
