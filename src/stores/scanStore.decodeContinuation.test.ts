...
function verifiedResponse(): Response {
  return {
    ...
    json: async () => ({
      ...
      results: [{
        productName: "Decode follow-up tire",
        brand: "Blackhawk",
        category: "Tire",
...
describe("trusted-exact decode continuation", () => {
  it("continues a trusted-exact miss on a non-GTIN code into ordinary decode", async () => {
  ...
  it("resolves the row honestly instead of stalling when the daily AI cap is genuinely spent during continuation", async () => {
    ...
    // server-side) so the handoff into ordinary decode is allowed, but the decode path's own
