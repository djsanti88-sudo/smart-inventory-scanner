// e2e/teach/lessons/7-live-decode-trace.mjs
//
...
// Proves the top-level law still holds through the decode decodeTrace (every
// scanned code appears + counts regardless of decode outcome), and surfaces
// non-locked diagnostic findings for the owner's decode-decodeTrace concerns
...
// changing decodeTrace logic itself; decodeTrace changes need owner approval.
//
...
// may or may not fire. To DETERMINISTICALLY exercise the decodeTrace, this lesson
...
import { evaluateDecodeTriggerOutcome } from '../decodeTrace.mjs';

export default {
  id: 'live-decode-trace',
  title: 'Live decode decodeTrace: every code still appears+counts; diagnose source behavior',
  level: 7,
  explore: false,
  ...
  async run(ctx) {
    ...
    if (unknownCodes.length === 0) {
      findings.push(triage.buildFinding({
        title: 'Persona has no unknown codes for live-decode-trace lesson',
        category: 'test_data',
        severity: 'medium',
        lesson: 'live-decode-trace',
        persona: persona?.key ?? null,
        repro: 'Inspect persona.codes.unknown.',
        expected: 'At least one unknown code available to exercise the decode decodeTrace.',
    ...
    const cap = h.attachDecodeTraceCapture(page, limits);
    ...
    try {
      for (const code of candidates) {
        ...
        // trigger the decodeTrace by clicking this row's manual "Look up with AI"
        ...
        // next scan, so the decodeTrace-capture attributes traces correctly.
    ...
    try {
      ...
      if (feedAfter < feedBefore + scannedCount) {
        findings.push(triage.buildFinding({
          ...
          lesson: 'live-decode-trace',
          persona: persona?.key ?? null,
          repro: `Scan ${scannedCount} unknown code(s) that route through the live decode decodeTrace: ${candidates.slice(0, scannedCount).join(', ')}.`,
          ...
          evidence: {},
          triageClass: 'probable_app_bug',
          customerImpact: 'A decode-decodeTrace outcome must never suppress a scanned row - top-level law violation with real customer scans.',
          locked: true,
        }));
        pass = false;
      }
    ...
    // the decodeTrace for the codes we attempted? Never silently pass when we
    // intended to decode but captured nothing - see the pure
    // evaluateDecodeTriggerOutcome() helper in decodeTrace.mjs.
    ...
    if (triggerOutcome.outcome === 'unavailable') {
      findings.push(triage.buildFinding({
        ...
        expected: 'The "Look up with AI" button is available and clickable so the decodeTrace can be exercised deterministically, OR the reason it is unavailable is an intentional, known gate (e.g. non-platform-owner identity, AI lookup off, import origin).',
        ...
        customerImpact: 'Diagnostic only - this run could not exercise the decode decodeTrace through the manual trigger; the decodeTrace itself is unverified for this run.',
        options: [
      ...
      findings.push(triage.buildFinding({
        title: `Clicked "Look up with AI" on ${triggerAvailableCount} code(s) but no /api/ai-lookup fired and no decodeTrace trace was captured`,
        category: 'decode-diagnosis',
        severity: 'high',
        lesson: 'live-decode-trace',
        persona: persona?.key ?? null,
        repro: `Scan unknown code(s) ${scannedCodes.join(', ')}, click data-testid="live-decode" on each row, observe network traffic.`,
        expected: 'Clicking "Look up with AI" issues a POST /api/ai-lookup request and the decodeTrace capture records a trace.',
        actual: `apiCalls=${apiCalls.length}, decodeTraceTraces=${rows.length} after clicking the trigger on ${triggerAvailableCount} available button(s).`,
        evidence: {},
        triageClass: 'probable_app_bug',
    ...
    for (const row of rows) {
      ...
      if (!row.reachedGpt && !row.gptSkipReason && row.settledSource && row.settledSource !== 'none' && row.settledSource !== 'gpt') {
        // Mirrors decodeTrace.mjs escapedGpt semantics per-row.
      }
    ...
    for (const row of rows) {
      ...
      if (escaped) {
        escapedGptCount += 1;
        findings.push(triage.buildFinding({
          title: `DecodeTrace ran a paid source for ${row.code} but never reached GPT with no honest skip reason`,
          category: 'decode-diagnosis',
          severity: 'medium',
          lesson: 'live-decode-trace',
          persona: persona?.key ?? null,
          repro: `Scan unknown code ${row.code} through the live decode decodeTrace and inspect debug.gptDecodeSkipReason / providerStatuses.`,
          expected: 'If a paid source ran, GPT is either reached or skipped with a recorded honest reason.',
          actual: `settledSource=${row.settledSource}, reachedGpt=false, gptSkipReason=null. reasons: ${row.reasonsSummary}`,
          evidence: {},
          triageClass: 'probable_app_bug',
          customerImpact: 'Diagnostic only - no customer-visible effect confirmed; flags a possible silent decodeTrace gap worth investigating.',
          options: [
            'Investigate why the GPT source was skipped without recording a reason for this code shape',
      ...
      if (row.partialIdentity) {
        findings.push(triage.buildFinding({
          ...
          lesson: 'live-decode-trace',
          persona: persona?.key ?? null,
          repro: `Scan unknown code ${row.code} through the live decode decodeTrace and inspect the returned identity (brand present, model+size missing).`,
          expected: 'A settled decode ideally includes model and size, not just brand, before being treated as a resolved identity.',
          actual: `identity had brand only; settledSource=${row.settledSource}, confidence=${row.confidence}.`,
          evidence: {},
          triageClass: 'probable_app_bug',
          customerImpact: 'Diagnostic only - partial identity may still route correctly to review/suggestion per existing rules; flags a decodeTrace-source investigation candidate.',
          options: [
            'Investigate why source ' + row.settledSource + ' settled without full specs for this code shape',
    ...
    notes.push(
      ...
      `${rows.length} decodeTrace trace(s), ${apiCalls.length} /api/* call(s) captured.`
    );
