...
an expert (Codex subagent) layer, and writes evidence to `reports/fable5/<run-id>/`.
...
- `python -m tools.fable5 review-build --gate fast` - run the deterministic check
  ...
  - `--with-experts` adds the Codex subagent layer (uses the subscription; never
...
Fable 5 experts run on the Codex subscription (not metered API billing) by default,
