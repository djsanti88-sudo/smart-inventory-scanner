...
const NOT_RUN = [
  ...
  // Both discovered 2026-09-08 (the TEST_FILE_RE widening below makes them visible for the
  // first time). Neither is a local-data gap -- both test an exported/CLI API the
  // corresponding script never grew (no export, no CLI flags beyond --force), so every test
  // fails before touching data.
  [
    "Retail knowledge builder (scripts/build-retail-knowledge.node-test.mjs)",
    "tests an exported buildRetailKnowledge({input, outputJson, receiptDir, mode: 'fixture'|'full', baselineJson, ...}) module API that scripts/build-retail-knowledge.mjs does not have -- needs a real refactor of the script to match the test's intended API.",
    "node --test scripts/build-retail-knowledge.node-test.mjs",
    ["scripts/build-retail-knowledge.node-test.mjs"],
  ],
  [
    "Knowledge DB builder (scripts/build-knowledge-db.node-test.mjs)",
    "tests a CLI accepting --tire-json/--output-db/--expected-tire-sha256/etc. that scripts/build-knowledge-db.mjs does not implement (it only reads --force and always writes the fixed committed path) -- needs a real refactor of the script to accept the flags the test already assumes.",
    "node --test scripts/build-knowledge-db.node-test.mjs",
    ["scripts/build-knowledge-db.node-test.mjs"],
  ],
];
