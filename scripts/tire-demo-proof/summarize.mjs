...
function readAnchoredEntries(root, batches, results, manifest) {
  return Array.from({ length: 30 }, (_, index) => {
    ...
    return { batch, result, anchor: { batch: batchNumber, manifest } };
  });
}
...
export function summarizeDirectory(runDirectory) {
  const root = resolve(runDirectory); const batches = join(root, "batches"); const results = join(root, "results"); const manifest = readManifestAnchor(root);
