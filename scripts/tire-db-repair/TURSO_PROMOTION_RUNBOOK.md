...
2. With the freeze active, run `backup`, then inspect its manifest. It must contain all five swapped
   ...
   recorded as `ABSENT`, not as empty tables.
3. Run `stage`, then `verify`, bound to that exact manifest. Stop on any failure; do not edit or
   regenerate staging files after backup without starting again at backup.
4. Run `promote` while the freeze remains active. Preserve the manifest, schema dump, JSONL dumps,
