...
describe("scripts/polish-backfill CLI", () => {
  it("prints usage and returns exit code 1 when --file is missing", () => {
    const logs = [];
  ...
  it("--dry-run prints changes but writes nothing", () => {
    const file = tmpFile("snap.json", { products: [product({ id: "p1" })] });
    const before = readFileSync(file, "utf8");
    const code = run(["--file", file, "--dry-run"]);
    expect(code).toBe(0);
    expect(readFileSync(file, "utf8")).toBe(before); // untouched
  });

  it("without --dry-run, writes structured fields in place (idempotent)", () => {
    const file = tmpFile("snap.json", { products: [product({ id: "p1" })] });
    run(["--file", file]);
  ...
  it("skips a product already stamped structuredBy human", () => {
  ...
  it("accepts a plain Product[] array and a raw persisted {state:{products}} snapshot", () => {
