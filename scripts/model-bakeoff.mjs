...
// and scores: got-size, size-correct, brand-correct, latency, est cost. Keys read from .env.local at runtime
// (never printed/committed). Usage: node scripts/model-bakeoff.mjs [--count=10]
...
const GEMINI_KEY = env.GEMINI_API_KEY, OPENAI_KEY = env.OPENAI_API_KEY;
...
// corpus ground truth: barcode -> {brand, model, size}
...
const sample = HARD.slice(0, COUNT).map((code) => ({ code, ...(truth.get(code) || {}) })).filter((t) => t.brand);
...
async function callModel(m, code, type) {
  const ctrl = new AbortController(); const to = setTimeout(() => ctrl.abort(), 45000);
  const p = prompt(code, type);
  try {
    if (m.provider === "gemini") {
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${m.id}:generateContent?key=${GEMINI_KEY}`;
    ...
    const r = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPENAI_KEY}` }, body: JSON.stringify({ model: m.id, messages: [{ role: "user", content: p }], response_format: { type: "json_object" } }), signal: ctrl.signal });
    const d = await r.json(); if (!r.ok) throw new Error(d.error?.message || `HTTP ${r.status}`);
    const u = d.usage || {};
...
(async () => {
  if (!GEMINI_KEY || !OPENAI_KEY) { console.error("missing GEMINI_API_KEY or OPENAI_API_KEY in .env.local"); process.exit(1); }
  ...
  for (const t of sample) {
    ...
    for (const m of MODELS) {
      const t0 = Date.now();
      try {
        const res = await callModel(m, t.code, type);
  ...
  console.log(`\nTotal bakeoff spend: $${total.toFixed(3)}  (cost numbers are ESTIMATES from public per-token pricing)`);
})();
